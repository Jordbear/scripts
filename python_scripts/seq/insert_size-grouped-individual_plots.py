...
for i in dfl:
    ...
    plot = sns.lineplot(x='insert_size', y='Reads', data=dfl[count])
    sns.despine()
    plot.set(xlim=(0, 750))
    plot.set(ylim=(0, None))
    plot.set_title(box[count]+' '+pos[count])
    def commas(x, pos):
        return '{:,}'.format(int(x))
    ax.get_yaxis().set_major_formatter(plt.FuncFormatter(commas))
    plt.savefig('insert_sizes_'+samples[count]+'.png', format='png', dpi=500, bbox_inches='tight')
    count+=1
...
fig = plt.figure(figsize=(7, 5))
ax = fig.add_subplot(1, 1, 1)
plot = sns.lineplot(x='insert_size', y='Reads', hue='box', units='sample', estimator=None, data=dfc)
sns.despine()
plot.set(xlim=(0, 750))
plot.set(ylim=(0, None))
...
def commas(x, pos):
    return '{:,}'.format(int(x))
ax.get_yaxis().set_major_formatter(plt.FuncFormatter(commas))
plt.savefig('insert_sizes-grouped_by_box.png', format='png', dpi=500, bbox_inches='tight')
