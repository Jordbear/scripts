...
names = [i.replace('_dm_mCtoT_all.mods', '') for i in names]
print(names)
print('')
...
count = 0
for i in dfl:
    dfl[count] = i[(i.START!=50) & (i.START!=110)]
    print(dfl[count])
    count+=1

count = 0
for i in dfl:
    dfl[count] = i.groupby('CONTEXT').aggregate('mean')
    print(dfl[count])
    count+=1
...
dfc['mod_rate'] = dfc['MOD']/(dfc['MOD']+dfc['UNMOD'])*100
print(dfc)
print('')

dfc.drop(['CHG', 'CHH', 'CN'], inplace=True)
print(dfc)
print('')
