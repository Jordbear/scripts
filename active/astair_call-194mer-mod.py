...
plot = sns.relplot(x='START', y='pos_mod_rate', kind='scatter', hue='GC', palette='coolwarm', col='sample', col_wrap=4, units='sample', linewidth=0, aspect=1.6, height=5, estimator=None, data=dfc_lambda)
count=0
...
fig = plt.figure(figsize=(7, 5))
ax = fig.add_subplot(1, 1, 1)
plot = sns.relplot(x='START', y='pos_mod_rate', kind='scatter', palette='coolwarm', col='sample', col_wrap=4, units='sample', linewidth=0, aspect=1.6, height=5, estimator=None, data=dfc_2kb)
count=0
for ax in plot.axes.flat:
    for i, spine in ax.spines.items():
        spine.set_visible(True)
...
fig = plt.figure(figsize=(7, 5))
ax = fig.add_subplot(1, 1, 1)
plot = sns.relplot(x='START', y='pos_mod_rate', kind='scatter', palette='coolwarm', col='sample', col_wrap=4, units='sample', linewidth=0, aspect=1.6, height=5, estimator=None, data=dfc_pUC19)
count=0
for ax in plot.axes.flat:
    for i, spine in ax.spines.items():
        spine.set_visible(True)
...
sum_lambda = dfc_lambda_final[['sample', 'sum_mod', 'sum_unmod', 'avg_read_mod_rate', 'avg_pos_mod_rate', '194mer_conversion', '194mer_calls']].drop_duplicates()
sum_lambda.reset_index(drop=True, inplace=True)
summary['lambda_mod_CpG'] = sum_lambda['sum_mod']
summary['lambda_unmod_CpG'] = sum_lambda['sum_unmod']
...
def mod_plot(metric):
    ...
    for p in plot.patches:
        if np.isnan(p.get_height())==False:
            if p.get_height()>=1:
                ...
                value_format = '{:.2f}'
            plot.text(p.get_x()+p.get_width()/2, p.get_height()+ax.get_ylim()[1]/200, value_format.format(p.get_height()), fontsize=50/len(names), ha='center')
    ax.set_ylim(top=490000)
    plot.set_xlabel('')
    ...
    def human_readable(x, pos):
        if len(str(int(x))) > 6:
            ...
            human_label = int(x)
        return human_label
    plot.get_yaxis().set_major_formatter(plt.FuncFormatter(human_readable))
    # for p in plot.patches:
    #     if np.isnan(p.get_height())==False and float('{:.5f}'.format(p.get_height())) in [95.57517, 95.78446, 95.56830, 95.72392, 95.45698, 95.61916, 95.59885, 95.69544]:
    #         print(p.get_x() + p.get_width() / 2)
    #         print(p.get_height())
